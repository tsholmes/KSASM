...
namespace KSASM.Assembly
{
  public class Parser : TokenProcessor
  {
    ...
    //       if (!valid)
    //         Invalid(token);
    //       if (TakeType(TokenType.Mult, out _))
    //       {
    //         if (!TakeType(TokenType.Number, out token))
    //           throw Invalid();
    //         if (!int.TryParse(buffer[token], out stmt.Width))
    //           Invalid(token);
    //       }
    //       Statements.Add(stmt);
    //     }
    ...
    //       Statements.Add(stmt);
    //     }
    //     else if (PeekType(TokenType.POpen, out _))
    //     {
